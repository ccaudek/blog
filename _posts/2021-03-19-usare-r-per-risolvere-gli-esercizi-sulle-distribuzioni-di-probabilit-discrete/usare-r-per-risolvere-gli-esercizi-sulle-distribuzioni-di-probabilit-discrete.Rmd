...
Consideriamo i problemi nei quali lo spazio campionario è costitutito da un insieme di eventi elementari aventi tutti la stessa probabilità di verificarsi. Utilizzeremo qui le funzioni fornite dal pacchetto `prob`.
...
Mediante la funzione `rolldie()`` è possibile generare lo spazio campionario per l'esperimento casuale consistente nel caso del lancio di un dado bilanciato:

```{r}
rolldie(1, makespace = TRUE)
```
Otteniamo un dataframe in cui ciascuna riga corrisponde ad un evento elementare. La colonna `probs` riporta la probabilità associata a ciascun evento elementare dello spazio campionario.

# Il lancio di due dadi

Ovviamente possiamo essere più ambiziosi e costruire, ad esempio, lo spazio campionario che consiste nel lancio di due dadi bilanciati:
...
Il dataframe $S$ è costituito da $6 \times 6$ righe:

```{r}
nrow(S)
```

Definiamo ora gli eventi $A$ e $B$. Siano $A$ e $B$ due eventi in $S$ tali per cui 
