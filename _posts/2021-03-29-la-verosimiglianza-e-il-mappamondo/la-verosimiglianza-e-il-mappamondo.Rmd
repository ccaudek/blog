...
Noi sappiamo quale sia la probabilità di sopravvivenza nella popolazione.
Ma possiamo calcolare la verosimiglianza di ottenere i dati che abbiamo osservato per valori diversi di questo parametro sconosciuto:

```{r}
dbinom(x=3, size=5, prob=0.1)
```

Se cambiamo il valore del parametro _p_ (probabilità di sopravvivenza), la verosimiglianza di ottenere i dati cambia:
